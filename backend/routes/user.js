...
module.exports = function (app) {
  app.post("/register", (req, res) => {
    ...
    db.query(
      "INSERT INTO usario (name, password, mail) VALUES (?,?,?)",
      [name, password, mail],
      (err, result) => {
        if (err) {
  ...
  app.post("/logout", (req, res) => {
    ...
    db.query(
      "UPDATE usario SET token = ? WHERE token = ?",
      [null, refreshToken],
      (err, result) => {
        if (err) {
  ...
  app.get("/refresh", (req, res) => {
    ...
    db.query(
      "SELECT * FROM usario WHERE token = ?",
      [refreshToken],
      (err, result) => {
        if (err) {
          ...
          jwt.verify(
            ...
            (err, decoded) => {
              ...
              if (err || !result[0] || result[0].id !== decoded.userid)
                return res.sendStatus(403);
  ...
  app.post("/login", (req, res) => {
    ...
    db.query("SELECT * FROM usario WHERE mail = ?", [mail], (err, result) => {
      if (err) {
        ...
        db.query(
          "UPDATE usario SET token = ? WHERE id = ?",
