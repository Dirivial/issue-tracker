...
function reducerFunction(state, action) {
  ...
  switch (action.type) {
    case "load":
      ...
      newState.sort((listA, listB) =>
        listA.position < listB.position ? -1 : 1
      );
      break;
    case "addList":
    ...
    case "addIssue":
      ...
      if (
        !newState[action.payload.listIndex].issues.find(
          (issue) => issue.id === action.payload.issue.id
        )
      ) {
        newState[action.payload.listIndex].issues = [
          ...state[action.payload.listIndex].issues,
    ...
    case "moveIssue":
      ...
      let issue = oldLists[source.droppableId].issues.splice(
        source.index,
        1
      )[0];
      ...
      if (source.droppableId === destination.droppableId) {
        ...
        for (
          let i = source.index;
          i < oldLists[source.droppableId].issues.length;
          i++
        ) {
        ...
        for (
          ...
          i < oldLists[destination.droppableId].issues.length;
          i++
        ) {
        ...
        oldLists[destination.droppableId].issues.splice(
          destination.index,
          0,
          issue
        );
